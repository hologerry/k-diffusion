...
def main():
    ...
    p.add_argument("prompt", type=str, default="the prompt to use")
    p.add_argument("--batch-size", type=int, default=16, help="the batch size")
    p.add_argument("--checkpoint", type=str, required=True, help="the checkpoint to use")
    p.add_argument("--clip-guidance-scale", "-cgs", type=float, default=500.0, help="the CLIP guidance scale")
    p.add_argument(
        "--clip-model", type=str, default="ViT-B/16", choices=clip.available_models(), help="the CLIP model to use"
    )
    p.add_argument("--config", type=str, help="the model config")
    ...
    p.add_argument("--steps", type=int, default=100, help="the number of denoising steps")
    args = p.parse_args()
    ...
    def run():
        if accelerator.is_local_main_process:
            tqdm.write("Sampling...")
        sigmas = K.sampling.get_sigmas_karras(args.steps, sigma_min, sigma_max, rho=7.0, device=device)

        def sample_fn(n):
            x = torch.randn([n, model_config["input_channels"], size[0], size[1]], device=device) * sigmas[0]
            x_0 = K.sampling.sample_dpmpp_2s_ancestral(
                model_fn, x, sigmas, eta=1.0, disable=not accelerator.is_local_main_process
            )
            return x_0
