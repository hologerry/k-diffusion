...
def make_model(config):
    ...
    if config["type"] == "image_v1":
        ...
        for depth, width, d_ff, self_attn, dropout in zip(
            config["depths"], config["widths"], config["d_ffs"], config["self_attns"], config["dropout_rate"]
        ):
        ...
        mapping = models.image_transformer_v2.MappingSpec(
            config["mapping_depth"], config["mapping_width"], config["mapping_d_ff"], config["mapping_dropout_rate"]
        )
